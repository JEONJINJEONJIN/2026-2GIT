...
CI note: BFI scores are repeated across scenario rows in `consistency_metrics.csv`, so paired scenario bootstrap intervals for BFI movement can be degenerate. This reflects the stored data shape, not independent BFI uncertainty.

## 2. BFI-Side Movement Results (Primary)

| Condition | Positive BFI cells | Mean delta BFI target | Largest absolute BFI movement |
|---|---:|---:|---:|
| `one_line_prompt` | 10 / 10 | +0.264 | 0.562 |
| `elaborate_prompt` | 10 / 10 | +0.127 | 0.344 |
| `as_pas_only` | 0 / 10 | +0.000 | 0.000 |
| `elaborate_prompt_as_pas` | 10 / 10 | +0.127 | 0.344 |

BFI-side movement is mainly prompt-driven. `as_pas_only` has zero BFI movement because it has no persona prompt, while prompt conditions move the self-report surface. This is why BFI must not be hidden inside a combined gap score.
