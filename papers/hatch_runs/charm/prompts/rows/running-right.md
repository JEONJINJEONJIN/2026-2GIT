Create a single horizontal sprite strip for the Codex app digital pet `charm` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Tiny original orange fire-lizard pet inspired by the references: round head, compact chibi body, cream belly patch, small arms and feet, big expressive dark eyes or sleepy closed eyes depending on pose, tapering tail with a small attached flame at the tip. Keep it legally distinct and original; do not recreate exact brand/model details..

Style contract: Codex digital pet sprite style: pixel-art-adjacent low-resolution mascot sprite, compact chibi proportions, chunky whole-body silhouette, thick dark 1-2 px outline, visible stepped/pixel edges, limited palette, flat cel shading with at most one small highlight and one shadow step, simple readable face, tiny limbs, and no detail that disappears at 192x208. Avoid polished illustration, painterly rendering, anime key art, 3D render, vector app-icon polish, glossy lighting, soft gradients, realistic fur or material texture, anti-aliased high-detail edges, and complex tiny accessories. Additional user style notes: 2D pixel art, Codex digital pet style, compact chibi proportions, thick dark 1-2 px outlines, limited orange/cream/yellow/red/brown/black palette, flat cel shading, crisp stepped pixel edges, no soft gradients, no shadows, no text..
